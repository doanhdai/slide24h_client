...
interface FeaturedSlidesWithCategoriesProps {
  slides: Slide[];
  categories?: string[];
}

export default function FeaturedSlidesWithCategories({
  slides,
  categories = ['Tất cả', 'Hot trend', 'Mẫu PPT', 'Intro/Outro', 'Game', 'Backdrop', 'Graphic', 'Morph'],
}: FeaturedSlidesWithCategoriesProps) {
  const [activeCategory, setActiveCategory] = useState('Tất cả');
  const [currentPage, setCurrentPage] = useState(0);

  // Filter slides based on active category
  // For now, show all slides when "Tất cả" is selected
  // In the future, you can add category field to slides
  const filteredSlides = activeCategory === 'Tất cả' 
    ? slides 
    : slides; // TODO: Implement actual category filtering when category data is available

  // Pagination: 9 slides per page
  ...
  const displayedSlides = filteredSlides.slice(startIndex, endIndex);
